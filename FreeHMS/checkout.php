   ...
   // process as form post
	...
	if( mysql_num_rows($result) > 0 )
		...
		for ($i=1; $i<=$_POST['num_rows']; $i++) {
				...
				$query = "SELECT calldate, dst, billsec FROM `cdr` WHERE dstchannel IS NOT NULL AND (lastapp = 'Dial' OR lastapp = 'ResetCDR') AND (dcontext = 'from-internal' OR dcontext = '" . $custchan . "' ) AND calldate BETWEEN '" . $Checkin . "' AND '" . $Checkout ."' AND channel LIKE '%/" . $Ext ."-%'";
