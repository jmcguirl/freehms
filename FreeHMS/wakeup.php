   ...
   // process as form post
	...
	function CheckWakeUp($file) {
		...
		$myfile =basename($file);
			...
			$myfile5 = $fileTmp[2];
			$myfile6 = $fileTmp[3];
			$fileTmp1 = explode("/", $myfile3);
			$myfile3 = $fileTmp1[5];
			$seter = 'Recep';
			
		if ($myfile3 == "wuc") {
		$myfile3 = date('Hi', $myfile4);
		$seter = 'Guest';
		}

//        echo "File " . $myfile3 . $myfile4 . $myfile5;  


	return array ($myfile3, $seter);
   	}
...
$myfile3 = CheckWakeUp($file);
	

		$myresult = $file;
		$h = substr($myfile3[0],0,2);
		$m = substr($myfile3[0],2,3);
		If ($myresult <> '') {
		 		echo "<TR><TD><FONT face=verdana,sans-serif>" . $row["ID"] . "</TD><TD>" . $h .":" . $m . "</TD><TD>" . $row["Desc"]  . "</TD><TD>" .$row["Ext"] ."</TD><TD>".$row["Name"]."</TD><TD><button NAME=\"DELETE\" TYPE=\"SUBMIT\" VALUE=\"". $myresult . "\" . \"> $h:$m Wakeup set by $myfile3[1]</button></TD>\n";
			}
		}
	}
echo "</TABLE>\n";
