...
echo "<center>" ;
echo "<h1>".$Title6."</h1>\n" ;
echo "The ".$SecLab31." column reflects the exact call cost at Checkout and wil not reflect any rate changes made on the system, The reflected cost is shown in the Report.";
...
echo "<TR><TD>Id</TD><TD>".$SecLab7."</TD><TD>".$SecLab14."</TD><TD>".$SecLab15."</TD><TD>".$SecLab17."</TD><TD>".$SecLab31."</TD><TD>".$SecLab1."</TD>" ;
	...
	mysql_close($dbconnection);
	while ($row = mysql_fetch_array($result)) {
 	echo "<TR><TD><FONT face=verdana,sans-serif>" . $row["ID"] . "</TD><TD>" . $row["Desc"]  . "</TD><TD>" .$row["Name"] ."</TD><TD>" . $row["Checkin"] . "</TD><TD>" . $row["Checkout"] . "</TD><TD>" . $row["Total"] . "</TD><TD><a href=\"ec.php?Name=" .$row["Name"] . "&Ext=" .$row["Ext"] . "&Checkin=" . $row["Checkin"] . "&Checkout=" . $row["Checkout"] . "&Total=" .$row["Total"] ."\">".$SecLab29."</a></TD></TR>\n" ;
	}
echo "</TABLE>\n";
endif;
